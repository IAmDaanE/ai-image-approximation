...
def run_training(image_path, epochs, start_lr):
    correct_image_pixels, scale, image_width, image_height = get_image(image_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Approximate an image using neural networks.")
    parser.add_index = parser.add_argument(
        "image_path", 
        type=str, 
        help="The path to the image file (e.g., ../images/hello_world.png)"
    )
    args = parser.parse_args()
    run_training(args.image_path, 5000000, 0.01)